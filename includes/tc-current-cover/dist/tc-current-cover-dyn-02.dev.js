...
var totalPremium = 0;
currentCover.insuranceProducts.forEach(function (product) {
...
function displayCoverInfo(coverType) {
  var product = currentCover.insuranceProducts.find(function (p) {
    return p.product.replace(/\s+/g, '') === coverType;
  });
  ...
  product.coverages.forEach(function (coverage) {
    ...
    value.textContent = "$".concat(coverage.benefit.toLocaleString());
...
currentCover.insuranceProducts.forEach(function (product, index) {
