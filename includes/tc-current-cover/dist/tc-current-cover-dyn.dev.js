...
accordionContainer.className = 'accordion-container'; // // Insurance cover data
// var currentCover = [
//     {
//         DeathCover: {
//             fixedCover: 261000,
...
function createAccordion(displayName, coverType) {
  ...
  accordionButton.className = 'accordion'; // Create a span element to wrap the icon and display name

  var accordionLabel = document.createElement('span');
  accordionLabel.className = 'accordion-label'; // Create an img element for the accordion icon
  ...
  accordionLabel.appendChild(accordionIcon); // Append the textContent (display name) to the label span

  accordionLabel.appendChild(document.createTextNode(displayName)); // Append the label span to the accordion button

  accordionButton.appendChild(accordionLabel); // Calculate and display the sum of fixedCover and ageBased
  ...
  accordionButton.appendChild(coverSumElement); // Set up the click event
...
function displayCoverInfo(coverType) {
  ...
  var panel = document.getElementById(coverType + 'Panel');
  panel.innerHTML = ''; // Iterate through cover properties and display in rows
...
function formatValue(property, value) {
  if (property === 'fixedCover') {
    ...
    return {
      label: 'Age-based',
      value: '$' + value.toFixed(2).replace(/\d(?=(\d{3})+\.)/g, '$&,')
    }; // return { label: 'Age-based', value: value };
...
accordionContainer.appendChild(deathCoverAccordion.panel); // Accordion for TPD Cover

var tpdCoverAccordion = createAccordion('TPD Cover', 'tpdCover');
accordionContainer.appendChild(tpdCoverAccordion.button);
accordionContainer.appendChild(tpdCoverAccordion.panel); // Accordion for IP Cover

var ipCoverAccordion = createAccordion('IP Cover', 'ipCover');
accordionContainer.appendChild(ipCoverAccordion.button);
accordionContainer.appendChild(ipCoverAccordion.panel); // Total Premium Row
...
accordionContainer.appendChild(totalPremiumRow); // Append the accordionContainer to the insuranceContainer

var insuranceContainer = document.getElementById('insuranceContainer');
insuranceContainer.appendChild(accordionContainer); // Display total premium
