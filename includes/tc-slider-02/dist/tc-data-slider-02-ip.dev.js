...
/*
    ...
    **  This script is called from "cover-state-death-edit.js"
    ...
    4. LISTENER SCRIPTS FOR SLIDER AND TEXT INPUT
        ...
        - textInputUpdatesIP()        ..... added to the html input returned from createSliderHTML
...
*/
// ---------------------------------------------------------------------
function sliderInitIP(productName) {
  ...
  var sliderContent = ''; // ADD slider html to 'sliderContent' by passing productName. 
  // NOTE: The "units" parameter seems redundant, but breaks code if not used - CLEAN UP?!

  sliderContent += createSliderHTML(productName, "units"); //  CREATE AN ARRAY

  var sliderReturn = [sliderContent, productName + "-slider", productName + "-tooltip", productName + "-text-input", productName + "-text-label", productName + "-item-value"]; // console.log("SliderReturn is: "+sliderReturn[0]);
...
function createSliderHTML(productName, sliderType) {
  ...
  if (sData.coverType === "Fixed") {
    ...
    } // oninput="textInputUpdatesIP('${productName}-text-input', '${productName}-text-input.value','${productName}-slider', '${productName}-tooltip', ${sData.min}, ${sData.max})"
  }
...
function sliderListenerIP(sliderId, tooltipId, textId, textLabelId, coverItemId, coverItemTotal) {
  //----- VARS -----
  // console.log("First text id check: " + textLabelId);
  var slider = document.getElementById(sliderId);
  var tooltip = document.getElementById(tooltipId);
  var textInputLabel = document.getElementById(textLabelId);
  var textInput = document.getElementById(textId); // let coverItemValue = document.getElementById(coverItemId);
  ...
  slider.addEventListener('input', function () {
    ...
    updateTooltipPositionIP(slider, tooltip);

    if (textInputLabel) {
      textInputLabel.innerHTML = slider.value / slider.step;
    }

    sessionStorage.setItem(sliderId, slider.value); // coverItemValue.innerHTML = formatCurrency(slider.value, 0); //death-fixed-item-value
...
function textInputUpdatesIP(senderId, value, sliderId, tooltipID, txtMin, txtMax, textLabelId, coverItemId) {
  ...
  // + "\nmax: " + max
  // )
  var txtInput = document.getElementById(senderId);
  var slider = document.getElementById(sliderId);
  var tooltip = document.getElementById(tooltipID);
  var textInputLabel = document.getElementById(textLabelId); // let coverItemValue = document.getElementById(coverItemId);
  ...
  if (txtInput.value <= txtMax && txtInput.value >= txtMin) {
    ...
    slider.value = txtInput.value; // console.log(document.getElementById("death-fixed-item-value").text);
    ...
    console.log("TEXT INPUT else CHANGE IS HERE!!!!!!!!" + formatCurrency(txtInput.value, 0)); // console.log("TtextInputLabel: "+ textInputLabel);
...
function updateTooltipPositionIP(slider, tooltip) {
  ...
  var tooltipLeft = sliderHandleLeft; // console.log("\nsliderRect width: "+ sliderRect.width
