// ---------------------------------------------------------------------
// ******** CUSTOM COVER – INCOME PROTECTION - STATE: EDIT COVER *********
// ---------------------------------------------------------------------
...
function ipEditInner(id){
    ...
    content += `<button class='secondary btn-medium btn-full'onclick='ipCurrentInner(ipBody);'>Reset IP cover</button>`
...
// ---------------------------------------------------------------------
function ipTotalPremium(coverArray){
...
function ipEditInnerContent(){
    ...
    <p class="caption">You canna start Basic cover without underwriting. This will not be offered again in the future.<br></p>`;
...
function createIPSliders(id){
    ...
    */
    ...
    // ----------------------------------------------------------------      
    let slider1 = sliderInit('ip-fixed');
    // Add slider html to document
    document.getElementById('slider-container-ip-fixed').innerHTML = slider1[0];
    ...
    function ipEditInnerFooterContent(aged, fixed){
        let premium = (Math.trunc(aged)*ipData.premiumBase)+(Math.trunc(fixed)*ipData.premiumBase);
