...
function ipCurrentOuterCard(id) {
  ...
  id.innerHTML += tcCard("ip", "default", "IP cover 2", "Find out more about cover types", "window.educovertypes.showModal();", "");
} // ---------------------------------------------------------------------
...
function ipCurrentInnerCard(id) {
  //  INNER CARD
  id.innerHTML += tcCard("ip-current", "bglight", "Your current IP benefit is:", "hide", "hidden education", "<button class='secondary btn-action danger' onclick=\"window.alert('Cancel this')\">Cancel</button>"); //  INNER BODY
...
var ipTotalPremiumCurrent = ipData.benefit * ipData.premiumBase;
...
function ipCurrentInnerContent() {
  ...
  content += createCoverItemSubText("", "Percent of salary", "".concat(percentOfSalary.toFixed(), "%"), tooltips["percent salary"]);
