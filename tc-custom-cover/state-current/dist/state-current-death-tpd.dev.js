...
function updateCoverDetails() {
  ...
  for (var key in deathCover) {
    if (deathCover.hasOwnProperty(key)) {
      ...
      var formattedValue = value; // Format as currency if the key is "Fixed Cover" or "Age-Based"
      ...
      coverHtml += '<div class="row">' + '<span class="label">' + '<img src="../images/icons/indent-arrow.svg" class="sub-icon" />' + '<a onclick=\'dictionary("' + key + '");\'>' + key + '</a>' + '</span>' + '<span class="cover-value">' + formattedValue + '</span>' + '</div>';
    }
...
function createCustomDeathTpdCurrent(id) {
  var parentId = id; // Iterating over each cover type
