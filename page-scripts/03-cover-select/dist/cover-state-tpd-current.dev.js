...
function tpdCurrentOuterCard(id) {
  ...
  id.innerHTML += tcCard("tpd", "default", "Total, Permanent Disablement (TPD)", "Find out more about cover types", "window.educovertypes.showModal();", ""); // space after the main card
...
function tpdCurrentInner(id) {
  ...
  content += tcCard("tpd-current", "bglight", "Your current TPD benefit is:", "hide", "hidden education", "<button class='secondary danger btn-action' onclick=\"window.alert('Cancel this')\">Cancel TPD</button>"); // 3 || ADD CONTENT AFTER INNER CARD
