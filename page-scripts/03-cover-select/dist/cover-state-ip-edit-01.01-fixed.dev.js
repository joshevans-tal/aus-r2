...
function ipEditInnerFixed(id) {
  ...
  content += tcCard("ip-edit", "bglight", "IP fixed benefit:", "hide", "window.alert('Education content about cover types')", "", "Initialise"); // 3 || ADD CONTENT AFTER INNER CARD
...
function ipEditInnerContentFixed() {
  ...
  // Total premium calc
  var ipPremiumCurrent = ipData.benefit * ipData.premiumBase + ipData.benefit * ipData.premiumBase;
  console.log('ipEditInnerConentFixed just ran'); // createIPIPSliders(ipBody);
...
function createIPSliders(id) {
  ...
  */
  ...
  var slider3 = sliderInitIP('ip-fixed'); // Add slider html to document

  document.getElementById('slider-container-ip-fixed').innerHTML = slider3[0]; // Add listeners using slider id and tooltip id

  sliderListenerIP(slider3[1], slider3[2], slider3[3], slider3[4], slider3[5]);
  console.log("\n                \n slider1[1]: ".concat(slider3[1], ",\n                \n slider1[2]: ").concat(slider3[2], ",\n                \n slider1[3]: ").concat(slider3[3], ",\n                \n slider1[4]: ").concat(slider3[4])); // ----------------------------------------------------------------
  // SET SESSION STORAGE - FOR SLIDER
  // ----------------------------------------------------------------

  sessionStorage.setItem(slider3[1], document.getElementById(slider3[1]).value); // console.log('currentTotalIPBenefit: '+currentTotalIPBenefit);
  // ----------------------------------------------------------------
  ...
  */
  ...
  updateIpFixedFooter(ipAAL); // vvvvvvvvvvvvvv TESTING THIS vvvvvvvvvvvvvvvvvv

  function updateCoverItemTotalsIP() {
    currentFixed = document.getElementById('ip-fixed-slider').value; // currentAged = sessionStorage.getItem('ip-aged');
  ...
  var ipFixedTxt = document.getElementById('ip-fixed-text-input');
  ...
  function ipEditInnerFooterContent(fixed) {
    var premium = Math.trunc(fixed) * ipData.premiumBase;
...
function updateFixedBenefit() {
  ...
  var formulaBenefitSelect = document.getElementById("ip-fixed-slider").value; // alert("Index: " + formulaBenefitSelect[x].index + " is " + formulaBenefitSelect[x].text + " and value is " + formulaBenefitSelect[x].value);
...
function getFixedBenfit() {
  ...
  var sliderValue = document.getElementById("ip-fixed-slider").value; // var formulaBenefitSelect = document.getElementById("ip-formula-benefit-select").options;
...
function updateFixedIpPremium() {
  var myBenefit = getFixedBenfit();
  updateFixedBenefit();
  var myBenefitPeriod = getFixedBenfitPeriod();
  var myWaitingPeriod = getFixedWaitingPeriod();
  var myMultiplier = ipMultiplierScheme[myBenefitPeriod][myWaitingPeriod];
  console.log("\n myMultiplier: " + myMultiplier + "\n myBenefit: " + myBenefit + "\n myBenefitPeriod: " + myBenefitPeriod + "\n myWaitingPeriod: " + myWaitingPeriod);
  var myPremium = myBenefit * myMultiplier;
...
function updateIpFixedFooter(benefit, waitingP, benefitP) {
  var premium = calcIPPremiumFixed(benefit);
...
function updateIPCartFooter(benefit, waitingP, benefitP) {
  var premium = calcIPPremiumFixed(benefit);
  document.getElementById('cart-ip-item-value').innerHTML = "".concat(formatCurrency(premium, 2), " /mth");
}
