...
function deathCurrentOuter(id) {
  ...
  id.innerHTML += tcCard("death", "default", "Death cover", "Find out more about cover types", "window.educovertypes.showModal();", "", ""); // space after the main card
...
function deathCurrentInner(id) {
  ...
  content += tcCard("death-current", "bglight", "Your current Death benefit is:", "hide", "education hidden", "<button class='secondary danger btn-action' onclick=\"window.alert('Cancel this')\">Cancel Death</button>", ''); // 3 || ADD CONTENT AFTER INNER CARD

  content += "<button class='primary btn-medium btn-full' onclick='deathEditInner(deathBody);'><img src=\"images/icons/ic-edit-white.svg\" /> Edit Death cover</button>\n    <div class=\"space v-small\"><!-- SPACER! --></div>\n    <button class='secondary btn-small btn-full' onclick=\"window.alert('Cancel this')\">Cancel Death cover</button>";
...
function deathCurrentInnerContent() {
  ...
  } // 04 || CREATE TOTAL PREMIUM LINE ITEM - CURRENT COVER


  content += "".concat(createCoverItemTotal("Monthly Premium", deathPremiumCurrent, "/mth")); // content += `<br>`;
  // 05 || SEND BACK THE CONTENT

  return content;
}

function deathCurrenInnerFooterContent() {
  var content = "<div class=\"footer-premium\">\n        <div>Monthly premium: <div>\n        <div>".concat(formatCurrency(deathPremiumCurrent, 0), "/mth</div>\n        <div>");
  return content;
}