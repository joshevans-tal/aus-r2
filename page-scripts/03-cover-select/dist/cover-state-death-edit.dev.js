...
function deathEditInner(id) {
  ...
  content += tcCard("death-edit", "bglight", "Edit your Death cover", "hide", "window.alert('Education content about cover types')", "", "Initialise"); // 3 || ADD CONTENT AFTER INNER CARD
...
var currentTotalDeathBenefit = updateTotalBenefit([deathDataAged.max, deathDataFixed.benefit]);
...
function deathEditInnerContent() {
  ...
  content += "\n    ".concat(createCoverItemDefault("Basic Death Cover", deathDataAged.coverType, deathDataAged.max, "Age-based changes your benefit amount with your age, while keeping your premiums more steady.", 'basic-death-aged')); // content += tcSwitch('death-aged-switch','Don\'t include', 'Include','round','');

  content += tcSwitch('death-aged-switch', 'Remove', 'Keep', 'round', 'checked'); // content += `<p class="caption" style="margin: 8px 16px;"><strong>Note:</strong> Basic Cover is only available when joining. It will not be offered again if it is removed.<br></p>`;

  content += "</div>";
  content += "<br><p style=\"margin-top: 16px;\"><strong>Add a <extra> fixed death cover</strong></p>"; // content += `<p class="caption">Unlike Basic Cover, adding extra fixed cover will require underwriting.<br></p>`
  ...
  content += "".concat(createCoverItemDefault("Death cover", 'Age-based', deathDataAged.benefit, // set to deathDataAged.max when basic cover is default checked
  deathDataAged.education, 'death-aged'));
  content += "".concat(createCoverItemDefault("Death cover", deathDataFixed.coverType, deathDataFixed.benefit, deathDataFixed.education, 'death-fixed'));
  content += "".concat(createCoverItemTotal('Total Death:', currentTotalDeathBenefit, '', 'total-death-value-item'), "\n        "); // content += `<div id="death-total-premium" class="total-premium">This is the total premium: ${deathPremiumCurrent}</div>`;
...
function createDeathSliders(id) {
  ...
  */
  ...
  sessionStorage.setItem(slider1[1], document.getElementById(slider1[1]).value); // console.log('currentTotalDeathBenefit: '+currentTotalDeathBenefit);
  // ----------------------------------------------------------------
  // LISTENER - DEATH AGED SWITCH
  // ----------------------------------------------------------------

  var checkbox = document.getElementById("death-aged-switch");
  var deathAgedCoverItem = document.getElementById('death-aged-item-value');
  var deathTotalCoverItem = document.getElementById('total-death-value-item');
  var deathCurrentInnerFooter = document.getElementById('death-edit-card-footer'); // checkbox.checked = true; // INCLUDE IF CHECKED

  updateCoverItemTotalsDeath();
  sessionStorage.setItem('death-aged', checkbox.checked); // // 4.1 || FILL INNER CARD FOOTER WITH CONTENT
  ...
  if (checkbox) {
    ...
    checkbox.addEventListener('change', function () {
      ...
      if (this.checked) {
        ...
        deathAgedCoverItem.innerHTML = formatCurrency('0', 2); // console.log('This is checkbox false/else condition');
  ...
  function updateCoverItemTotalsDeath() {
    ...
    var totalDeath = Math.trunc(currentFixed) + Math.trunc(currentAged);
  ...
  */
  ...
  var deathFixedSlider = document.getElementById('death-fixed-slider');
  deathFixedSlider.addEventListener('input', function () {
    updateCoverItemTotalsDeath();
    console.log('This is my new listener');
  });
  var deathFixedText = document.getElementById('death-fixed-text-input');
  ...
  function deathEditInnerFooterContent(aged, fixed) {
    // Calculate premium
    var premium = Math.trunc(aged) * deathDataAged.premiumBase + Math.trunc(fixed) * deathDataFixed.premiumBase; // Add to session storage
