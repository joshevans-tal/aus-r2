...
function ipEditInnerFormula(id) {
  ...
  content += tcCard("ip-edit", "bglight", "IP formula benefit:", "hide", "window.alert('Education content about cover types')", "", "Initialise"); // 3 || ADD CONTENT AFTER INNER CARD
...
function ipEditInnerContentFormula() {
  ...
  content = "<div><h3 id=\"ip-benefit-agedbased\">".concat(formatCurrency(ipAAL, 0), " /mth</h3></div>");
  content += "Select a new IP benefit amount\n                <div class=\"space v-small\"><!-- SPACER! --></div>";
  content += "<select class=\"tc-select\" style=\"display:block; width:100%;\" id=\"ip-formula-benefit-select\" onchange=\"updateFormulaIpPremium()\">\n                    <option value=\"".concat(ipAAL, "\">Keep existing</option>\n                    <option value=\"").concat(ipEmployerSalaryMaxMth, "\">Increase to maximum</option>\n                </select>\n                <div class=\"space v-medium\"><!-- SPACER! --></div>");

  function selectForm(data, selected, id) {
    // label
    content += "<div>".concat(data[0], "</div> \n        <div class=\"space v-small\"><!-- SPACER! --></div>");
    content += "<div><select class=\"tc-select\" style=\"display:block; width:100%;\" id=\"".concat(id, "\" name=\"").concat(id, "\">");

    for (var _i = 0; _i < data[1].length; _i++) {
      if (data[1][_i] == selected) {
        content += "<option value=\"".concat(data[1][_i], "\" selected>").concat(data[1][_i], "</option>");
      } else {
        content += "<option value=\"".concat(data[1][_i], "\">").concat(data[1][_i], "</option>");
      }
    } // close off
...
function updateFormulaBenefit() {
  ...
  var formulaBenefitSelect = document.getElementById("ip-formula-benefit-select").options; // alert("Index: " + formulaBenefitSelect[x].index + " is " + formulaBenefitSelect[x].text + " and value is " + formulaBenefitSelect[x].value);
...
function updateFormulaIpPremium() {
  var myBenefit = getFormulaBenfit();
  updateFormulaBenefit();
  var myBenefitPeriod = getFormulaBenfitPeriod();
  var myWaitingPeriod = getFormulaWaitingPeriod();
...
function updateIpFormulaFooter(benefit, waitingP, benefitP) {
  var premium = calcIPPremiumFormula(benefit);
