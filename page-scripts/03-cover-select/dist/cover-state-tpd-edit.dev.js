...
function tpdEditInner(id) {
  ...
  var content = '';
  content += "<div class=\"space v-medium\"></div>"; // 2 || CREATE INNER CARD

  content += tcCard("tpd-edit", "bglight", "Edit your TPD cover", "hide", "window.alert('Education content about cover types')", "", "Initialise"); // 3 || ADD CONTENT AFTER INNER CARD

  content += "<button class='secondary btn-medium btn-full'onclick='tpdCurrentInner(tpdBody);'><img src=\"images/icons/ic-reset.svg\" /> Reset to existing cover</button>";
  id.innerHTML = content;
  var tpdEditBody = document.getElementById('tpd-edit-card-body');
...
var currentTotalTPDBenefit = updateTotalBenefit([tpdDataAged.max, tpdDataFixed.benefit]);
...
function tpdEditInnerContent() {
  ...
  // Total premium calc
  var tpdPremiumCurrent = tpdDataFixed.benefit * tpdDataFixed.premiumBase + tpdDataAged.benefit * tpdDataAged.premiumBase; // createTPDTPDSliders(tpdBody);
  ...
  content += "\n    ".concat(createCoverItemDefault("Basic TPD Cover", tpdDataAged.coverType, tpdDataAged.max, "Age-based changes your benefit amount with your age, while keeping your premiums more steady.", 'basic-tpd-aged')); // content += tcSwitch('tpd-aged-switch','Don\'t include', 'Include','round','');

  content += tcSwitch('tpd-aged-switch', 'Remove', 'Keep', 'round', 'checked'); // content += `<p class="caption" style="margin: 8px 16px;"><strong>Note:</strong> Basic Cover is only available when joining. It will not be offered again if it is removed.<br></p>`;

  content += "</div>";
  content += "<br><p style=\"margin-top: 16px;\"><strong>Add a <extra> fixed tpd cover</strong></p>"; // content += `<p class="caption">Unlike Basic Cover, adding extra fixed cover will require underwriting.<br></p>`
  ...
  content += "".concat(createCoverItemDefault("TPD cover", 'Age-based', tpdDataAged.benefit, // set to tpdDataAged.max when basic cover is default checked
  tpdDataAged.education, 'tpd-aged'));
  content += "".concat(createCoverItemDefault("TPD cover", tpdDataFixed.coverType, tpdDataFixed.benefit, tpdDataFixed.education, 'tpd-fixed'));
  content += "".concat(createCoverItemTotal('Total TPD:', currentTotalTPDBenefit, '', 'total-tpd-value-item'), "\n        "); // content += `<div id="tpd-total-premium" class="total-premium">This is the total premium: ${tpdPremiumCurrent}</div>`;
...
function createTPDSliders(id) {
  ...
  */
  ...
  // ----------------------------------------------------------------      
  var slider1 = sliderInit('tpd-fixed'); // Add slider html to document

  document.getElementById('slider-container-tpd-fixed').innerHTML = slider1[0]; // Add listeners using slider id and tooltip id
  ...
  sessionStorage.setItem(slider1[1], document.getElementById(slider1[1]).value); // console.log('currentTotalTPDBenefit: '+currentTotalTPDBenefit);
  // ----------------------------------------------------------------
  // LISTENER - TPD AGED SWITCH
  // ----------------------------------------------------------------

  var checkbox = document.getElementById("tpd-aged-switch");
  var tpdAgedCoverItem = document.getElementById('tpd-aged-item-value');
  var tpdTotalCoverItem = document.getElementById('total-tpd-value-item');
  var tpdCurrentInnerFooter = document.getElementById('tpd-edit-card-footer'); // checkbox.checked = true; // INCLUDE IF CHECKED

  updateCoverItemTotalsTPD();
  sessionStorage.setItem('tpd-aged', checkbox.checked); // // 4.1 || FILL INNER CARD FOOTER WITH CONTENT
  ...
  if (checkbox) {
    ...
    checkbox.addEventListener('change', function () {
      ...
      if (this.checked) {
        ...
        tpdAgedCoverItem.innerHTML = formatCurrency('0', 2); // console.log('This is checkbox false/else condition');
  ...
  function updateCoverItemTotalsTPD() {
    ...
    var totalTPD = Math.trunc(currentFixed) + Math.trunc(currentAged);
  ...
  */
  ...
  var tpdFixedSlider = document.getElementById('tpd-fixed-slider');
  tpdFixedSlider.addEventListener('input', function () {
    updateCoverItemTotalsTPD();
    console.log('This is my new listener');
  });
  var tpdFixedText = document.getElementById('tpd-fixed-text-input');
