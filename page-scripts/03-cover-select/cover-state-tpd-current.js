...
function tpdCurrentInner(id){
    ...
    content += tcCard(
        ...
        `hide`,
        `hidden education`,
        `<button class='secondary danger btn-action' onclick="window.alert('Cancel this')">Cancel TPD</button>`
        );
    
    // 3 || ADD CONTENT AFTER INNER CARD
    content += `<button class='primary btn-medium btn-full'onclick='tpdEditInner(tpdBody);'><img src="images/icons/ic-edit-white.svg" /> Edit TPD cover</button>`
    id.innerHTML = content;
    
    // 4 || FILL INNER CARD BODY WITH CONTENT
    // Get the reference id
    let tpdCurrentBody = document.getElementById('tpd-current-card-body');
