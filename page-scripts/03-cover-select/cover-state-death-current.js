...
function deathCurrentInner(id){
    ...
    content += `<p>If you wish to keep this existing cover, you don’t need to do anything.<br><br></p>`;
    ...
    content += tcCard(
        ...
        `hide`,
        `education hidden`,
        `<button class='secondary danger btn-action' onclick="window.alert('Cancel this')">Cancel Death</button>`,
        ''
        );
    ...
    // 4 || FILL INNER CARD BODY WITH CONTENT
    // Get the reference id
    let deathCurrentBody = document.getElementById('death-current-card-body');
...
function deathCurrentInnerContent(){
    ...
    switch(deathDataAged.benefit){
        ...
        default:
            ...
            deathDataAged.benefit,
            tooltips["age-based"])}`;
    }
    
    // 04 || CREATE TOTAL PREMIUM LINE ITEM - CURRENT COVER
    content += `${createCoverItemTotal(
            "Monthly Premium",
            deathPremiumCurrent,
            "/mth")}`;
...
function deathCurrenInnerFooterContent(){
    ...
    let content = `<div class="footer-premium">
        ...
        <div>${formatCurrency(deathPremiumCurrent,0)}/mth</div>
        <div>`;
