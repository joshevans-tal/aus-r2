...
function initCurrentCover() {
    ...
    let coverTotals = `<div id="cover-totals-container">`;
    coverTotals += `<h3 class="make-flex">NEW total premium</h3>`;
    coverTotals += createCoverItemDefault(
        "Death Cover",
        '',
        deathPremiumCurrent,
       '',
    ...
    bodyContainer.innerHTML += coverTotals;

    // bodyContainer.innerHTML += 'some more text';
    bodyContainer.innerHTML += tcCTA(
        ...
        [
            [
                ...
                'large',
                'Confirm & continue',
                `window.location = ("04-confirm-cover-latest.html")`
            ],
            [
                'tertiary',
                'large',
...
// ---------------------------------------------------------------------
let newDeathPremium = 0;
let newTPDPremium = 0;
let newIPPremium = 0;
let newTotalPremium = 0;

let newDeathPremiumId = document.getElementById('cart-death-item-value');
let newTPDPremiumId = document.getElementById('cart-tpd-item-value');
let newIPPremiumId = document.getElementById('cart-ip-item-value');
let newTotalPremiumId = document.getElementById('cart-total-cover');

function updateCartTotals(){

    newDeathPremiumId.innerHTML = `${formatCurrency(newDeathPremium)} /mth`;
    sessionStorage.setItem('new-death-premium',newDeathPremium);

    newTPDPremiumId.innerHTML = `${formatCurrency(newTPDPremium)} /mth`;
    sessionStorage.setItem('new-tpd-premium',newTPDPremium);

    newIPPremiumId.innerHTML = `${formatCurrency(newIPPremium)} /mth`;
    sessionStorage.setItem('new-ip-premium',newIPPremium);

    newTotalPremiumId.innerHTML = `${formatCurrency(newDeathPremium+newTPDPremium+newIPPremium)} /mth`;
