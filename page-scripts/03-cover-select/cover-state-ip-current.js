...
function ipCurrentInnerCard(id){
    ...
    content += tcCard(
        ...
        `hide`,
        `hidden education`,
        `<button class='secondary btn-action danger' onclick="window.alert('Cancel this')">Cancel IP</button>`
        );
...
// ---------------------------------------------------------------------
let ipCurrentMultiplier = 0.9576*ipMultiplier;
console.log('hello - can you read the multiplier? it is: '+ipCurrentMultiplier);
let ipTotalPremiumCurrent = ipData.aal*ipCurrentMultiplier;
...
function ipCurrentInnerContent(){
    ...
    content += createCoverItemDefault("Income Protection","formula",ipData.aal,tooltips["formula"]);
